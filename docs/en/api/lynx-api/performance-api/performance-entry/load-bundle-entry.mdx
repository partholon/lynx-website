...
_LoadBundle_ refers to the initial frame rendering pipeline triggered by [LynxView](/guide/spec#lynxview) loading and executing the [TemplateBundle](/api/lynx-native-api/template-bundle). `LoadBundleEntry` is used to record the performance data of this rendering pipeline and inherits from [`PipelineEntry`](./pipeline-entry).
